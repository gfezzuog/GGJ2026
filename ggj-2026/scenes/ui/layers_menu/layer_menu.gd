...
func create_masks():
	for i in range(mask_textures.size()):
		...
		if coordinates != []:
			var packed_mask: PackedScene = load("res://scenes/components/mask/mask.tscn")
...
func _on_mask_dragged(value: bool, mask: Mask):
	if value == true:
		var packed_dragged_mask: PackedScene = load("res://scenes/ui/layers_menu/dragged_mask.tscn")
