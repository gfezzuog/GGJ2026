...
func create_masks():
	for i in range(mask_textures.size()):
		...
		if coordinates != []:
			var packed_mask: PackedScene = load("res://scenes/old_components/mask/mask.tscn")
			var new_mask: OldMask = packed_mask.instantiate()
...
func _on_mask_dragged(value: bool, mask: OldMask):
	if value == true:
		var packed_dragged_mask: PackedScene = load("res://scenes/old_ui/layers_menu/dragged_mask.tscn")
